...
export function ResourceCard({ resource, compact = false, bonusPerRound = 0 }: ResourceCardProps) {
  const config = RESOURCE_CONFIG[resource.resource_type];
  if (!config) return null;

  const totalPerRound = resource.replenish_per_round + bonusPerRound;
    ...
    <div className="relative overflow-hidden bg-card/70 border border-[rgba(0,240,255,0.06)] rounded-2xl p-4 hover:border-[rgba(0,240,255,0.15)] transition-all duration-500 group hover:shadow-[0_0_20px_rgba(0,240,255,0.08)]">
      ...
      <div className="flex items-end justify-between">
        ...
        <span className="text-[10px] font-mono text-muted">
          +{totalPerRound}/rd
          {bonusPerRound > 0 && (
            <span className="ml-1 text-neon-lime" style={{ textShadow: `0 0 6px ${config.color}30` }}>
              (+{bonusPerRound})
            </span>
          )}
        </span>
